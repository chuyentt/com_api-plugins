...
```http
POST /index.php?option=com_api&app=articles&resource=article&format=raw&key=<key>
```
OR update an existing article
```http
POST /index.php?option=com_api&app=articles&resource=article&format=raw&key=<key>&id=:id
```
